-- Better subsidiary matching: "VECTREN CORPORATION" should hit alias "Vectren",
-- "Luminant Energy Company LLC" should hit "Luminant". Plain trigram similarity
-- penalizes length gaps; word_similarity scores an alias embedded in a longer
-- name as ~1.0. Length guard (>=6) keeps short strings from substring-junk.
  ...
  with hits as (
    select e.id, e.canonical_name, e.ticker, e.cik, e.entity_type, e.data_tier, e.hq_state,
           greatest(
             ...
             case when length(e.canonical_name) >= 6 then word_similarity(e.canonical_name, q) else 0 end
           ) as score,
           null::text as matched_alias
    from public.entities e
    where (e.created_by_tenant is null or e.created_by_tenant = public.current_tenant_id())
      and (e.canonical_name % q or (length(e.canonical_name) >= 6 and e.canonical_name <% q))
    union all
    select e.id, e.canonical_name, e.ticker, e.cik, e.entity_type, e.data_tier, e.hq_state,
           1.0, null::text
    from public.entities e
